    ...
    <div class="max-w-3xl mx-auto bg-gray-900 rounded-xl shadow-2xl border border-gray-800 overflow-hidden">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 md:p-8">
        ...
        <div class="flex flex-col space-y-2">
          <label class="text-sm text-gray-400 font-medium">FC Máxima</label>
          <div class="relative">
            <input 
...
const fcTrabajo = computed(() => {
  return Math.round(fcMax.value * intensidad.value)
})
...
const cargarHistorial = async () => {
  try {
    ...
    // Filtramos para que aquí solo aparezcan los registros de "FCTrabajo"
    historial.value = datos.filter(item => item.tipo === 'FCTrabajo')
  } catch (error) {
