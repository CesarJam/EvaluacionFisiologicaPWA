<template>
  <div class="mt-8 px-4">
    ...
    <div class="max-w-3xl mx-auto bg-gray-900 rounded-xl shadow-2xl border border-gray-800 overflow-hidden">
      ...
      <div class="bg-gray-800 p-6 md:p-8 flex flex-col sm:flex-row justify-between items-center border-t border-gray-700 gap-4">
        <div class="text-center sm:text-left">
          ...
          <span class="block text-xl text-white font-semibold">FC Máxima</span>
        </div>
        
        <!-- Caja del número (Inspirada en el amarillo de tu tabla original) -->
    ...
    <!-- Fórmula / Ayuda visual -->
    <p class="mt-6 text-sm text-gray-500 text-center max-w-xl mx-auto flex items-center justify-center gap-2">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      Fórmula: Frecuencia cardiaca teórica - Edad
    </p>
        ...
        <table class="w-full text-left text-sm text-gray-400">
          ...
          <tbody>
            <tr v-for="item in historial" :key="item.id" class="border-b border-gray-800 hover:bg-gray-800 transition-colors">
              ...
              <td class="p-4 text-center">{{ item.parametros.edad }} años</td>
...
const abrirModal = (id) => {
  ...
  mostrarModal.value = true // Muestra el fondo oscuro y la ventana
}

const cerrarModal = () => {
  mostrarModal.value = false
  idAEliminar.value = null // Limpiamos el ID seleccionado
}
...
const confirmarBorrado = async () => {
  if (idAEliminar.value !== null) {
    try {
      ...
      cerrarModal() // Cerramos la ventana emergente
    } catch (error) {
...
const fcMax = computed(() => {
  return fcReferencia.value - edad.value
})

// Variable para almacenar los registros de la base de datos
const historial = ref([])

// Función para cargar los datos desde SQLite
const cargarHistorial = async () => {
  try {
    const datos = await obtenerHistorial()
    // Filtramos para mostrar solo los de Astrand en esta vista (opcional, pero recomendado)
    historial.value = datos.filter(item => item.tipo === 'Astrand')
  } catch (error) {
    console.error("Error al cargar el historial:", error)
  }
}

// Cargar el historial en cuanto la página se renderiza
onMounted(() => {
  cargarHistorial()
})
...
const guardarDatos = async () => {
  ...
  try {
    await guardarEvaluacion('Astrand', props.nombre || 'Sin nombre', parametros, fcMax.value)
    await cargarHistorial() // <-- Recargamos la tabla automáticamente después de guardar
    //alert('¡Evaluación guardada en SQLite con éxito!')
  } catch (error) {
...
const compartirHistorialWhatsApp = () => {
  ...
  let mensaje = `*LIONSSS ACADEMY* 🦁\n📋 *Historial de Evaluaciones (Astrand)*\n\n`;
  
  // Recorremos el historial y armamos el texto
  historial.value.forEach((item, index) => {
    ...
    mensaje += `🎯 FC Máx: ${item.resultado} lat/min\n`;
    mensaje += `--------------------------\n`;
  });

  mensaje += `\nCreado y desarrollado por CesarJam94`;

  // Codificamos el texto y abrimos WhatsApp
  const url = `https://api.whatsapp.com/send?text=${encodeURIComponent(mensaje)}`;
  window.open(url, '_blank');
}
</script>